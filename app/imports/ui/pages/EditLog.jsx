...
const EditLog = ({ logId }) => {
  ...
  const editLog = () => {
    WorkoutLogs.collection.update({ _id: logId }, (error) => {
      if (error) {
        swal('Error', error.message, 'error');
      } else {
        swal('Success', 'Workout Log Updated successfully', 'success');
      }
    });
    setShow(false);
  };
    ...
    <div id={ComponentIDs.editLog}>
      ...
      <Modal show={show} onHide={handleClose}>
        ...
        <Modal.Footer>
          ...
          <Button variant="outline-danger" size="sm" onClick={editLog}>Update</Button>
        </Modal.Footer>
      </Modal>
    </div>
  );
